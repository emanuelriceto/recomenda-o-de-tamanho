"""
FASE 1B — Size Charts das Marcas (3 Modelagens)
TCC: Sistema de Recomendação de Tamanho para Vestuário Superior

Modelagens suportadas (alinhadas com o data.yaml do YOLO):
...
# ── Modelagens suportadas e ajustes relativos ao regular ─────
# (delta_largura_busto, delta_ombro, delta_comprimento)
AJUSTES = {
    'regular':   ( 0.0,  0.0,  0.0),
    'slim':      (-2.0, -0.5,  0.0),  # corte mais justo
    'oversized': (+6.0, +2.0, +8.0),  # propositalmente largo
}

MODELAGENS = list(AJUSTES.keys())
...
MARCAS = [
    ...
    },
    {
        ...
        'observacao': 'Marca internacional, tabela para Brasil',
        'tamanhos_regular': [
...
def gerar_entradas(tamanhos_regular: list) -> list:
    """
    Gera entradas para as 3 modelagens aplicando ajustes ao regular.
    """
    todas = []
    for modelagem, (d_bust, d_ombro, d_compr) in AJUSTES.items():
...
def popular_size_charts():
    ...
    for marca_data in MARCAS:
        ...
        # Remover entradas anteriores desta marca para recriar limpas
        cur.execute('DELETE FROM tabela_tamanhos WHERE marca_id = ?', (marca_id,))

        entradas = gerar_entradas(marca_data['tamanhos_regular'])
...
def consultar_tamanho(busto_corpo: float, modelagem: str = 'regular',
    ...
    """
    Consulta o tamanho recomendado para um dado busto e modelagem.
    """
    conn = sqlite3.connect(DB_PATH)
    cur  = conn.cursor()
    ...
    print(f'\n📏 busto={busto_corpo}cm | modelagem={modelagem}'
    ...
    for row in resultados:
        ...
        print(f'{nome:<12} {label:<8} {mod:<12} '
              f'{bc_min}–{bc_max} cm{"":>6} {ombro} cm{"":>3} {compr} cm')
