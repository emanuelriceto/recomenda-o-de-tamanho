"""
FASE 2C — Treinamento do Modelo ML com Modelagem como Feature
TCC: Sistema de Recomendação de Tamanho para Vestuário Superior

ATUALIZAÇÃO: 'modelagem_num' adicionada como feature de entrada.
O XGBoost agora recebe:
  - medidas corporais do cliente
  - modelagem detectada pelo YOLO (0=regular, 1=slim, 2=oversized,
                                   3=longline, 4=henley)
E retorna o tamanho recomendado com confiança.
"""
...
MODELAGEM_MAP = {
    "regular": 0, "slim": 1, "oversized": 2,
    "longline": 3, "henley": 4,
}

# ── Features de treino ────────────────────────────────────────
# ATUALIZAÇÃO: modelagem_num adicionada
FEATURES_PRINCIPAIS = [
    "busto_circunf",     # mais importante — circunferência do tórax
    ...
    "modelagem_num",     # NOVO: 0=regular, 1=slim, 2=oversized, 3=longline, 4=henley
]
...
def treinar_comparar(X, y, le):
    ...
    for nome, pipeline in modelos.items():
        ...
        resultados[nome] = {
            "pipeline": pipeline, "cv_acc": cv_acc.mean(),
            "cv_acc_std": cv_acc.std(), "cv_f1": cv_f1.mean(),
            "cv_f1_std": cv_f1.std(), "f1_test": f1_test,
            "acc_test": acc_test, "y_pred": y_pred,
        }
...
def gerar_graficos(resultados, melhor_nome, X_test, y_test, le, features):
    ...
    ax.set_title("Comparação de Modelos (com feature modelagem)",
                 fontweight="bold")
    ax.legend()
    plt.tight_layout()
    ...
    ax.set_title("Importância das Features (Random Forest)\n"
                 "Amarelo = modelagem_num (nova feature)",
                 fontweight="bold")
...
def prever_tamanho(busto_circunf: float, largura_ombro: float,
                   ...
                   genero: str, modelagem: str = "regular",
                   cintura_circunf: float = None,
                   comprimento_braco: float = None) -> dict:
    """
    Predição de tamanho dado medidas corporais + modelagem detectada pelo YOLO.
    ...
    Parâmetros:
      ...
      peso_kg        : peso em kg
      genero         : 'M' ou 'F'
      modelagem      : 'regular', 'slim', 'oversized', 'longline' ou 'henley'
                       (detectada automaticamente pelo YOLO na foto)
    """
    ...
    mod_num  = MODELAGEM_MAP.get(modelagem, 0)
...
def treinar_modelo():
    print("\n" + "="*60)
    print("  FASE 2C — Treinamento do Modelo ML (com Modelagem)")
    print("="*60)
    ...
    # Teste ao vivo com modelagem
    print("="*60)
    print("  TESTE AO VIVO")
    print("="*60)
    for mod in ["regular", "slim", "oversized"]:
        r = prever_tamanho(94.0, 43.0, 175.0, 78.0, "M", modelagem=mod)
        print(f"\n  busto=94cm | ombro=43cm | modelagem={mod}")
        print(f"  → Tamanho: {r['tamanho_recomendado']} "
              f"(confiança: {r['confianca']:.1%})")
